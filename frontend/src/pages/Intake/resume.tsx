 ...
 */
export function mapDraftToFormData(draft: Record<string, unknown>): Partial<IntakeFormData> {
  // Support both nested format {company: {name, ...}, icp: {...}}
  // and flat format {company_name, website, icp_industries, ...} (from auto-save)
  const isNested = typeof draft.company === "object" && draft.company !== null;

  if (!isNested) {
    // Flat format — the draft IS already IntakeFormData, return it directly
    const flat = draft as Partial<IntakeFormData>;
    return { ...flat, negative_icp_confirmed_empty: flat.negative_icp_confirmed_empty ?? null };
  }

  // Nested format — map into flat IntakeFormData
