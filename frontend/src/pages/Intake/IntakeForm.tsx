...
import { z } from "zod";
import { mapDraftToFormData } from "./resume";
...
export default function IntakeForm() {
  const { step, setStep, formData, rehydrate } = useIntakeStore();
  ...
  // Auto-rehydrate from saved draft on first mount if the form is still empty
  useEffect(() => {
    if (formData.company_name) return; // already populated, skip
    const clientId = getActiveClientId();
    loadDraft(clientId)
      .then((data) => rehydrate(mapDraftToFormData(data as Record<string, unknown>)))
      .catch(() => {}); // 404 = no draft, nothing to restore
