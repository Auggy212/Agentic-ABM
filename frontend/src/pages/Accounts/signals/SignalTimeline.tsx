      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: "var(--text-0, #111827)" }}>
              {TYPE_LABELS[signal.type] ?? signal.type.replace(/_/g, " ")}
            </span>
          </div>
          <div style={{ fontSize: 13, color: "var(--text-0, #111827)", lineHeight: 1.4 }}>
            {signal.description}
          </div>
        </div>
        <div style={{ flexShrink: 0, textAlign: "right" }}>
          <div
            style={{ fontSize: 12, color: "var(--text-1, #374151)", fontWeight: 500 }}
            title={absoluteTime(signal.detected_at)}
          >
            {relativeTime(signal.detected_at)}
          </div>
          <div style={{ fontSize: 11, color: "var(--text-2, #6b7280)", marginTop: 2 }}>
