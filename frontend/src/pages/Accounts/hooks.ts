...
function buildQueryParams(clientId: string, filters: AccountsFilters, page = 1, pageSize = 50) {
  return {
    client_id: clientId,
...
export function useAccounts({
  ...
  return useQuery({
    queryKey: ["accounts", clientId, filters, page, pageSize],
    queryFn: async () => {
      const { data } = await api.get<AccountsListResponse>("/api/accounts", {
        params: buildQueryParams(clientId, filters, page, pageSize),
      });
