  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      <div className="card card-pad" style={{ borderRadius: 8, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <select className="abm-input" value={filters.validation} onChange={(e) => setFilters({ ...filters, validation: e.target.value as CP3Filters["validation"] })} style={{ width: 210 }}>
          <option value="ALL">All — any validation status</option>
          <option value="HARD_FAIL">⚠ Hard Fail — untraced claim</option>
          <option value="SOFT_FAIL">⚡ Soft Fail — weakly sourced</option>
          <option value="DIVERSITY_COLLISION">🔁 Collision — too similar to another</option>
          <option value="ALL_PASS">✓ Clean — no issues</option>
        </select>
        <select className="abm-input" value={filters.reviewState} onChange={(e) => setFilters({ ...filters, reviewState: e.target.value as CP3Filters["reviewState"] })} style={{ width: 160 }}>
          <option value="ALL">All — any decision</option>
