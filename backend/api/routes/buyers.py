...
POST /api/buyers/discover                    â€" trigger BuyerIntelAgent run (async)

GET  /api/buyers?company={domain}            â€" BuyerProfiles for one domain

GET  /api/buyers?client_id={uuid}            â€" full BuyerIntelPackage for a client

GET  /api/buyers/contact/{contact_id}        â€" single contact detail

PATCH /api/buyers/contact/{contact_id}       â€" operator manual edit (CP2 audit)
...
class DiscoverRequest(BaseModel):
    ...
    zero_credit_mode: bool = Field(False, description="Skip reveal flags and verifiers — 0 Apollo credits used, returns work emails only")
...
async def _run_buyer_intel(client_id: str, run_id: str, zero_credit_mode: bool = False) -> None:
    ...
    await agent.run(
        ...
        zero_credit_mode=zero_credit_mode,

    )



    # RAG: index buyer profiles
    ...
    try:
        ...
        try:
            ...
            post_run_index(_db, client_id, ["buyer_profiles"])
...
async def discover(
    ...
    Runs asynchronously â€" returns a job_id immediately. Poll the run record
    ...
    background_tasks.add_task(_run_buyer_intel, body.client_id, run_id, body.zero_credit_mode)
...
def list_buyers(
    ...
    - ?client_id={uuid}   â†’ full BuyerIntelPackage (all accounts)

    - ?company={domain}   â†’ list of BuyerProfile for that single domain
    ...
    if company:
        ...
        if not records:
            ...
            return {"domain": company, "contacts": [], "total": 0}
...
def edit_contact(
    ...
    if body.email is not None:
        ...
        data["email_status"] = "UNVERIFIED"   # reset â€" Verifier will re-check
