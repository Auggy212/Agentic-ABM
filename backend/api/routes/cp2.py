...
def post_auto_approve(
    ...
    """Bulk-approve all pending claims/accounts and approve CP2 in one call. Used by auto-pilot mode."""
    try:
        state = state_manager.auto_approve_cp2(client_id=client_id, reviewer=reviewer, db=db)
    except CP2StateError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    return _state_response(state)
