...
POST /api/signals/discover                        â€” trigger SignalIntelAgent run (async)
GET  /api/signals?company={domain}               â€” SignalReport for one domain
GET  /api/signals?client_id={uuid}               â€” all SignalReports for a client
POST /api/signals/{domain}/regenerate-intel       â€” regenerate intel report for one Tier 1 account
"""
...
async def _run_signal_intel(client_id: str, run_id: str, domain_filter: Optional[str] = None) -> None:
    ...
    # RAG: index signal reports
    try:
        from backend.agents.copilot.rag_indexer import post_run_index
        from backend.db.session import SessionLocal
        _db = SessionLocal()
        try:
            post_run_index(_db, client_id, ["signal_reports"])
        finally:
            _db.close()
    except Exception:
        pass
...
def list_signals(
    ...
    - ?client_id={uuid}   â†’ map of domain â†’ SignalReport for all accounts
    - ?company={domain}   â†’ single SignalReport for that domain
    """
...
async def regenerate_intel(
    ...
    Operator override â€” ignores token budget for this one call.
    """
    ...
    if account_record.tier != AccountTier.TIER_1.value:
        raise HTTPException(
            status_code=400,
            detail=f"Account {domain!r} is {account_record.tier} â€” intel reports are Tier 1 only.",
        )
