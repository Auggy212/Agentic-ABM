...
def auto_approve_cp3(
    client_id: str = Query(...),
    reviewer: str = Query(DEFAULT_REVIEWER),
    db: Session = Depends(get_db),
) -> Dict[str, Any]:
    """Bulk-approve all pending messages/buyers and approve CP3 in one call. Used by auto-pilot mode."""
    try:
        state = state_manager.auto_approve_cp3(client_id=client_id, reviewer=reviewer, db=db)
    except CP3StateError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    return state.model_dump(mode="json")
