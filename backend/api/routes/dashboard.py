...
def get_nav_counts(
    client_id: str = Query(..., description="Client UUID of the logged-in user"),
    db: Session = Depends(get_db),
) -> Dict[str, Any]:
    """
    Get navigation counts for dashboard, scoped to the authenticated user's client_id.
    ...
    sequences_count = db.query(func.count(MessageRecord.id)).filter(
...
def get_pipeline_status(
    ...
    cp2_status = cp2_record.status if cp2_record else "NOT_STARTED"
...
def _agent_state(record, *, total: int) -> Dict[str, Any]:
    ...
    if status in {"COMPLETE", "SUCCESS", "COMPLETE_WITH_WARNINGS"}:
        status = "COMPLETED"
    if status == "RUNNING":
