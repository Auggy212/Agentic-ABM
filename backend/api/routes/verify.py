...
async def _run_verify(client_id: str, run_id: str, contact_id: str | None = None) -> None:
    ...
    # RAG: index verification results
    try:
        from backend.agents.copilot.rag_indexer import post_run_index
        from backend.db.session import SessionLocal
        _db = SessionLocal()
        try:
            post_run_index(_db, client_id, ["verification_results"])
        finally:
            _db.close()
    except Exception:
        pass
...
def get_latest_package(
    ...
    # Enrich each verification row with display_name and source from buyer profiles
    contact_ids = [str(v["contact_id"]) for v in result.get("verifications", [])]
    if contact_ids:
        profiles = (
            db.query(BuyerProfileRecord)
            .filter(
        ...
        profile_map = {
            str(p.contact_id): {
                "display_name": (p.data or {}).get("full_name") or (p.data or {}).get("first_name", "") + " " + (p.data or {}).get("last_name", ""),
                "source": (p.data or {}).get("source") or "apollo",
                "title": (p.data or {}).get("current_title") or (p.data or {}).get("title") or "",
            }
            for p in profiles
        }
        for v in result.get("verifications", []):
            cid = str(v["contact_id"])
            info = profile_map.get(cid, {})
            v["display_name"] = (info.get("display_name") or "").strip() or None
            v["source"] = info.get("source") or "apollo"
            v["contact_title"] = info.get("title") or ""
