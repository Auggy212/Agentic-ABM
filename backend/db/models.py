...
class IntakeDraftRecord(Base):
    """
    Persists the user's in-progress intake form fields to SQLite so drafts
    survive Redis restarts. One row per client_id; upserted on every auto-save.
    """
...
class PipelineRunRecord(Base):
    """Persisted pipeline run status — survives server restarts."""
    ...
    completed_steps: Mapped[dict] = mapped_column(JSON, nullable=False, default=list)
...
class UserRecord(Base):
    """Registered users. Each user owns a unique client_id for data isolation."""
    ...
    client_id: Mapped[str] = mapped_column(String(36), nullable=False, unique=True, default=lambda: str(uuid.uuid4()))
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False,
        default=lambda: datetime.now(tz=timezone.utc),
...
class VisualAssetRecord(Base):
    """
    Campaign visual assets generated by VisualAgent via Canva MCP.

    One row per generated asset. channel drives design_type selection
    (EMAIL → email, LINKEDIN → instagram_post, etc.). status tracks the
    Canva generation lifecycle: PENDING → GENERATING → READY | FAILED.
    canva_design_id holds the Canva design ID once created from the candidate.
    """

    __tablename__ = "visual_assets"
    __table_args__ = (
        Index("ix_visual_assets_client_status", "client_id", "status"),
        Index("ix_visual_assets_message", "message_id"),
        Index("ix_visual_assets_client_account", "client_id", "account_domain"),
    )
    ...
    canva_design_id: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    canva_edit_url: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    canva_view_url: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
# ---------------------------------------------------------------------------
# RAG Copilot — new tables
# ---------------------------------------------------------------------------

class CopilotConversationRecord(Base):
    """
    Persisted copilot conversation history per client.

    Each row is one turn (role=user|assistant). The copilot route fetches
    the last N turns as the rolling context window.
    """

    __tablename__ = "copilot_conversations"
    __table_args__ = (
        Index("ix_copilot_conv_client_created", "client_id", "created_at"),
    )
...
class RagIndexRecord(Base):
    ...
    One row per indexed document. The embedding is stored as a JSON-encoded
    list of floats (portable across SQLite and PostgreSQL without extensions).
    Source tables: icp_accounts, buyer_profiles, signal_reports, messages,
    verification_results, sales_handoffs, master_contexts, cp3_client_feedback.
    """

    __tablename__ = "rag_index"
    __table_args__ = (
        Index("ix_rag_index_client_source", "client_id", "source_table"),
        Index("ix_rag_index_source_row", "source_table", "source_id", unique=True),
    )
    ...
    # Human-readable text that was embedded — also used for keyword search fallback
    text_chunk: Mapped[str] = mapped_column(Text, nullable=False)
    # JSON-encoded float list: "[0.123, -0.456, ...]"
