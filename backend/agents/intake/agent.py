...
class IntakeAgent:
    ...
    def validate(self, raw_submission: dict) -> ValidationResult:
        ...
        Steps:
        1. Coerce obvious fixable values (URL scheme, whitespace, etc.)
        2. Pydantic structural validation (field types, enums, required fields)
        3. Vague-value heuristics that produce clarifying questions
        ...
        # Step 1 — coerce fixable values so Pydantic doesn't hard-fail
        raw_submission = self._coerce(raw_submission)
    ...
    def build_master_context(self, validated: dict, *, client_id: Optional[str] = None) -> MasterContext:
        """
        ...
        If client_id is provided (e.g. the authenticated user's client_id), it is
        used directly so all pipeline data stays under the same identity.
        """
        now = datetime.now(tz=timezone.utc)
        enriched = self._coerce(dict(validated))

        # Strip internal confirmation flag — not part of the schema
        enriched.pop("negative_icp_confirmed_empty", None)

        resolved_client_id = client_id if client_id else str(uuid.uuid4())
        enriched["meta"] = {
    ...
    def _coerce(self, raw: dict) -> dict:
        """
        Normalise obviously fixable values before Pydantic sees them, so that
        minor formatting issues don't hard-block submission.
        """
        import copy
        ...
        # website: add https:// if scheme is missing
