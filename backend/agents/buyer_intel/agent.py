...
class BuyerIntelAgent:
    ...
    async def run(
        ...
        # Map ICP seniority labels to Apollo's seniority strings (hyphens → underscores)
        ...
        for account in accounts_sorted:
            ...
            try:
                ...
                quota_warnings.append({
                    ...
                    "note": f"Apollo rate limit (429) hit at domain={domain}; stopping run. Wait 1 hour before retrying.",
                })
                pending_domains.append(domain)
                remaining = [a.domain for a in accounts_sorted
                             if a.domain not in package_accounts and a.domain != domain]
                pending_domains.extend(remaining)
                logger.warning("BuyerIntelAgent: Apollo rate limit hit at domain=%s — aborting run", domain)
                break
