...
def auto_approve_cp3(client_id: str, reviewer: str, db: Session) -> CP3ReviewState:
    """Bulk-approve all pending messages and buyers, bypass client review, then approve CP3. Used by auto-pilot mode."""
    try:
        record = _record(db, client_id)
        state = _state(record)
    except CP3NotFoundError:
        state = open_review(client_id, reviewer, db)
        record = _record(db, client_id)

    now = _now()

    new_reviews = [
        r.model_copy(update={"review_decision": MessageReviewDecision.APPROVED, "reviewed_at": now, "review_notes": "Auto-approved"})
        if r.review_decision == MessageReviewDecision.PENDING else r
        for r in state.message_reviews
    ]
    new_buyers = [
        b.model_copy(update={"buyer_decision": BuyerDecision.APPROVED, "buyer_notes": "Auto-approved"})
        if b.buyer_decision == BuyerDecision.PENDING else b
        for b in state.buyer_approvals
    ]

    updated = state.model_copy(update={
        "message_reviews": new_reviews,
        "buyer_approvals": new_buyers,
        "operator_completed_at": now,
        "client_completed_at": now,
        "reviewer": reviewer,
    })
    _persist(db, record, updated)
    _audit(db, client_id=client_id, action="AUTO_APPROVE_CP3", reviewer=reviewer)
    return approve_cp3(client_id, "Auto-approved by pipeline", reviewer, db)
