...
def auto_approve_cp2(
    ...
    """Bulk-approve all pending claims and accounts, then approve CP2. Used by auto-pilot mode."""
    try:
        record = _load_record(db, client_id)
        state = _state_from_record(record)
    ...
    new_claims = [
        c.model_copy(update={"review_decision": ReviewDecision.APPROVED, "reviewed_at": now, "review_notes": "Auto-approved"})
        if c.review_decision == ReviewDecision.PENDING else c
        for c in state.inferred_claims_review
    ]
    new_accounts = [
        a.model_copy(update={"account_decision": AccountDecision.APPROVED, "buyer_profiles_approved": True, "account_notes": "Auto-approved"})
        if a.account_decision == AccountDecision.PENDING else a
        for a in state.account_approvals
    ]

    updated = state.model_copy(update={
        "inferred_claims_review": new_claims,
        "account_approvals": new_accounts,
        "reviewer": reviewer,
    })
    _persist(db, record, updated)
    _audit(db, client_id=client_id, action="AUTO_APPROVE_CP2", reviewer=reviewer)
    return approve_cp2(client_id, reviewer, db, reviewer_notes="Auto-approved by pipeline")
